...
function initMotionPlannerCore (context) {
  ...
  {
    this.Stack.push({x: Math.trunc(position.x * this._Step_Scale.x), y: Math.trunc(position.y * this._Step_Scale.y)});
  }
  ...
  {
    var current_vel = Math.sqrt( Math.pow((this.CurrentVelocity.x / this._Step_Scale.x), 2) + Math.pow((this.CurrentVelocity.y / this._Step_Scale.y), 2));
    console.log("Cartesion Vel: " + current_vel);
    return current_vel;
  }
  MotionPlanner.prototype.set_feedrate = function(feed)
  {
    if (this.Motion.dx > this.Motion.dy)
    {
      //X is our big axis
      this._Feedrate_delay = this.Motion.dx;
    }
    else
    {
      //Y is our big axis
  ...
  MotionPlanner.prototype.motion_tick = function()
  {
    ...
    {
      if (Date.now() > this._Feed_Sample_Timestamp + 100) //Sample every 100 milliseconds
      {
        //Calculate the distance traveled per second for each axis
        this.CurrentVelocity.x = Math.abs(this.LastPosition.x - this.CurrentPosition.x) * 10;
        this.CurrentVelocity.y = Math.abs(this.LastPosition.y - this.CurrentPosition.y) * 10;
        //console.log("X Vel: " + this.CurrentVelocity.x + " Y Vel: " + this.CurrentVelocity.y);
        this.get_current_velocity();
        this.LastPosition = this.copy_obj(this.CurrentPosition);
        this._Feed_Sample_Timestamp = Date.now();
      }
      if (this.CurrentPosition.x != this.TargetPosition.x || this.CurrentPosition.y != this.TargetPosition.y)
      {
