...
function initMotionPlannerCore (context) {
  ...
  */
  ...
  {
    //First pass - Iterate all the moves on the stack and calculate the ramp_maps in exact stop mode
    ...
    for (var x = 0; x < this.Stack.length; x++)
    {
      ...
      move.ramp_map = this.motion_calculate_ramp_map(Math.abs(current_position.x - move.x) / this._Step_Scale.x, Math.abs(current_position.y - move.y) / this._Step_Scale.x, move.f);
  ...
  */
  MotionPlanner.prototype.motion_recalculate_ramp_map_for_exit = function(ramp, exit_velocity)
  {
    ...
    console.log("Decel starts at: " + biggest_feed_index + " Decel ends at: " + exit_velocity_index);
  }
  /*
  ...
  */
  ...
  MotionPlanner.prototype.motion_tick = function()
  {
    ...
    {
      if (Date.now() > this._Feed_Sample_Timestamp + 50) //Sample every x milliseconds, calculate the percentage of move complete and set the feedrate to the pre-calculated feedrate map
      {
        if (this.CurrentPosition.x != this.TargetPosition.x || this.CurrentPosition.y != this.TargetPosition.y)
        {
          ...
          var percentage_into_move = Math.trunc(this.map(dominent_stg / dominent_axis_steps, 1, 0, 0, 999));
          //console.log("Percentage through move: " + percentage_into_move);
          this.set_feedrate(this.Motion.ramp_map[percentage_into_move]);
      ...
      if (Date.now() > this._Feedrate_Timestamp + this._Feedrate_delay)
      {
        ...
        else
        {
          if (this.Stack.length > 0) //There are pending moves on the stack!
          {
            ...
            this.Motion.ramp_map = next_move.ramp_map;
