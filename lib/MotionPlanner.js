...
function initMotionPlannerCore (context) {
  ...
  MotionPlanner.prototype.motion_set_target = function(x, y)
  {
    ...
    this.Motion.x_stg = this.copy_obj(this.Motion.dx);
    this.Motion.y_stg = this.copy_obj(this.Motion.dy);
  }
  MotionPlanner.prototype.motion_tick = function()
  {
    if (this.Motion.x_stg > 0 || this.Motion.y_stg > 0)
    {
      ...
      var e2 = this.Motion.err;
      if (e2 > -this.Motion.dx) { this.Motion.err -= this.Motion.dy; this.CurrentPosition.x += this.Motion.sx; this.Motion.x_stg--; this.Callbacks.Step_X(); }
      if (e2 < this.Motion.dy) { this.Motion.err += this.Motion.dx; this.CurrentPosition.y += this.Motion.sy; this.Motion.y_stg--; this.Callbacks.Step_Y(); }
    }
    else
    {
